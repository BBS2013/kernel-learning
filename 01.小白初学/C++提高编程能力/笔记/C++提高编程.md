...
int main()
{
	...
	for (int i = 0; i < 35; ++i) {
		iv.push_back(i);
		if (iv.capacity() != vector_capacity)
		{
			vector_capacity = iv.capacity();
			cout << "vector's capacity has changed" << endl;
		}
		cout << "iv.capacity()： " << iv.capacity() << endl;
		cout << "iv.size()： " << iv.size() << endl << endl;
	}
...
![image-20210920214304601](assets/image-20210920214304601.png)

这是capacity的增长轨迹，是2的幂次增长，是指数爆炸，不会导致增长假象。当然，开始时容量较小可能会重新分配几次内存，不过容量小也就代表开销小，无伤大雅。至于迭代器，基本上都是随用随取，不会有失效问题，除非多线程。如果一定要保留元素的位置，不应该保留迭代器，而是下标。所以问题是，到底reserve多大的初始值才能降低重新分配的频率又不浪费空间，而不是越大越好。 其实，如果不采取任何措施，20次重新分配内存就可以容纳100万元素了，如果初始reserve(8)，那就是800万，完全不用担心vector的效率。

当到达下一个底临界点时，你插入一个数据时，就需要先复制100多万个数据到新内存，然后在插入新的数据，这个成本其实是很大的。数据量小时，一般编程是不用关心的，但需要了解下原理
